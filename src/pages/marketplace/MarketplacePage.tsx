...
export default function MarketplacePage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8">
        <h1 className="mb-2 text-2xl font-bold text-raycast-text">能力市场</h1>
        <p className="text-raycast-muted">浏览和发现 DEX 插件、MCP 工具与 Skill</p>
      </div>

      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative flex-1 max-w-md">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-raycast-dim" />
          <input
            type="text"
            placeholder={placeholderText}
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full rounded-lg border border-[#252829] bg-raycast-surface py-2 pl-10 pr-4 text-sm text-raycast-text placeholder:text-raycast-dim focus:border-primary-500 focus:outline-none"
          />
        </div>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-medium ${
              section === 'plugins'
                ...
                : 'bg-raycast-elevated text-raycast-muted hover:brightness-110'
            }`}
          >
        ...
        filteredMcp.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filteredMcp.map((tool) => (
              <div
                key={tool.name}
                className="group rounded-xl border border-[#252829] bg-raycast-surface p-6 transition-all hover:border-primary-200"
              >
                <div className="mb-4 flex items-start justify-between">
                  <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-raycast-elevated text-xl">
                    {MCP_ICONS[tool.name] || '🔌'}
                  </div>
                  <span className="rounded-full bg-raycast-elevated px-2.5 py-0.5 text-xs font-medium text-raycast-blue">
                    MCP
                  </span>
                </div>
                ...
                <p className="mb-4 line-clamp-2 text-sm text-raycast-muted">{tool.description}</p>

                {tool.parameters?.properties && (
                  <div className="space-y-1">
                    <p className="text-xs font-medium text-raycast-dim">参数：</p>
                    {Object.entries(tool.parameters.properties).map(([key, prop]) => (
                      <div key={key} className="flex items-center gap-2 text-xs text-raycast-muted">
                        <code className="rounded bg-raycast-elevated px-1.5 py-0.5 font-mono text-raycast-text">
                          {key}
                        </code>
              ...
              >
                <div className="mb-4 flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-semibold text-raycast-text">{skill.name}</h3>
                      {'cron' in skill && (
                        <span className="rounded bg-raycast-elevated px-1.5 py-0.5 text-[10px] font-medium text-purple-400">
                          定时
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-raycast-muted">{skill.description}</p>
                    {'cron' in skill && (
                ...
                <div className="relative mb-4">
                  {skill.steps.map((step, i) => {
                    const label = FUNCTION_LABELS[step.function] || { name: step.function, icon: '⚡', color: 'bg-raycast-elevated text-raycast-text' }
                    return (
                      <div key={i} className="flex items-start gap-3">
                        {i > 0 && (
                          <div className="absolute left-[15px] top-0 h-4 w-0.5 -translate-y-3 bg-[#252829]" style={{ marginTop: i === 1 ? '-4px' : '0' }} />
                        )}
                        <div className="flex flex-col items-center">
                          <span className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold ${label.color}`}>
                            {label.icon}
                          </span>
                          {i < skill.steps.length - 1 && <div className="h-4 w-0.5 bg-[#252829]" />}
                        </div>
                        <div className="flex-1 pt-0.5">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium text-raycast-text">{label.name}</span>
                            <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${
                              step.type === 'mcp' ? 'bg-raycast-elevated text-raycast-blue' : 'bg-raycast-elevated text-green-400'
                            }`}>
                          ...
                          <div className="mt-0.5 flex flex-wrap gap-1">
                            {Object.entries(step.args).map(([k, v]) => (
                              <span key={k} className="rounded bg-raycast-surface px-1.5 py-0.5 text-[11px] font-mono text-raycast-dim">
                ...
                <div className="flex gap-2">
                  ...
                  <button className="rounded-full bg-[hsla(0,0%,100%,0.815)] px-4 py-2 text-sm font-medium text-[#18191a]">
                    详情
                  </button>
                </div>
              </div>
