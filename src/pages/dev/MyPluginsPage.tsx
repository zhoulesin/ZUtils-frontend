  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-raycast-text">我的插件</h1>
        <Link
          to={ROUTES.DEV_PLUGINS_NEW}
          className="inline-flex items-center gap-1 rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white hover:bg-primary-500"
        >
      ...
      {plugins.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-xl border-raycast-border-solid bg-raycast-surface">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-raycast-elevated text-left text-raycast-muted">
            ...
            <tbody>
              {plugins.map((p) => (
                <tr key={p.id} className="border-b last:border-0 hover:bg-raycast-elevated">
                  <td className="px-4 py-3 font-medium text-raycast-text">{p.functionName}</td>
                  <td className="px-4 py-3 text-raycast-muted">{CATEGORY_LABELS[p.category] ?? p.category}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <Link
                        ...
                        className="inline-flex items-center gap-1 text-raycast-muted hover:underline"
                      >
