...
export default function NewVersionPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-6 text-2xl font-bold text-raycast-text">发布新版本</h1>
      <form onSubmit={handleSubmit} className="space-y-5 rounded-xl border-raycast-border-solid bg-raycast-surface p-6">
        <div>
          <label className="mb-1 block text-sm font-medium text-raycast-muted">版本号 *</label>
          <input
            value={version}
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-raycast-muted">实现类全限定名 *</label>
          <input
            value={className}
            onChange={(e) => setClassName(e.target.value)}
            className="w-full rounded-lg border border-raycast-border-solid px-4 py-2 text-sm font-mono text-sm focus:border-primary-500 focus:outline-none"
            placeholder="com.example.GetWeatherFunction"
            required
          />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-raycast-muted">DEX 文件 *</label>
          <div className="mt-1 flex items-center justify-center rounded-lg border-2 border-dashed border-raycast-border-solid px-6 py-8">
            <div className="text-center">
              <Upload className="mx-auto mb-2 h-8 w-8 text-raycast-dim" />
              {file ? (
                <p className="text-sm text-raycast-muted">{file.name} ({(file.size / 1024).toFixed(1)} KB)</p>
              ) : (
                <>
                  <p className="text-sm text-raycast-muted">拖拽或点击选择 DEX 文件</p>
                  <input
                    type="file"
                    accept=".dex"
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-raycast-muted">更新日志</label>
          <textarea
            value={changelog}
